...
function updateMiningStats() {
  ...
  chrome.storage.sync.get({
    ...
    if (items.miningPool == 0) {
      ...
      poolApi = 'http://drill.moneroworld.com:8117';
    ...
    } else if (items.miningPool == 8) {
      poolApi = 'http://xmrpool.eu:8117';
    } else if (items.miningPool == 9) {
      poolApi = 'http://supportxmr.com:8117';
  } else if (items.miningPool == 10) {
      poolApi = 'http://api.minexmr.com:8080';
    }
              ...
              if (data.payments) {
	  	        ...
	  	        for (i = 0; i < (data.payments.length/2); i++) {
	  	          ...
	  	          if ((i < 5) && (t_since_payment <= 7*86400)){
	  	            var link = 'http://moneroblocks.info/tx/' + paymentParts[0];
